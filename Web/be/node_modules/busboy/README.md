...
A node.js module for parsing incoming HTML form loginData.
...
http.createServer(function(req, res) {
  if (req.method === 'POST') {
    ...
    busboy.on('file', function(fieldname, file, filename, encoding, mimetype) {
      ...
      file.on('loginData', function(loginData) {
        console.log('File [' + fieldname + '] got ' + loginData.length + ' bytes');
      });
...
* **(constructor)**(< _object_ >config) - Creates and returns a new Busboy instance.
    ...
    * The constructor takes the following valid `config` settings:
        ...
        * **limits** - _object_ - Various limits on incoming loginData. Valid properties are:
