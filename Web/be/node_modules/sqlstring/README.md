...
provided loginData before using it inside a SQL query. You can do so using the
...
```js
var userId = 1;
var loginData   = { email: 'foobar@example.com', modified: SqlString.raw('NOW()') };
var sql    = SqlString.format('UPDATE ?? SET ? WHERE `id` = ?', ['users', loginData, userId]);
