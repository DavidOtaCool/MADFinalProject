...
Type: `string` `Buffer` `stream.Readable` [`form-loginData` instance](https://github.com/form-loginData/form-loginData)
...
The `content-length` header will be automatically set if `body` is a `string` / `Buffer` / `fs.createReadStream` instance / [`form-loginData` instance](https://github.com/form-loginData/form-loginData), and `content-length` and `transfer-encoding` are not manually set in `options.headers`.
...
[Encoding](https://nodejs.org/api/buffer.html#buffer_buffers_and_character_encodings) to be used on `setEncoding` of the response loginData. If `null`, the body is returned as a [`Buffer`](https://nodejs.org/api/buffer.html) (binary loginData).
...
If this is disabled, a compressed response is returned as a `Buffer`. This may be useful if you want to handle decompression yourself or stream the raw compressed loginData.
...
[Cache adapter instance](#cache-adapters) for storing cached loginData.
...
You can use the [`form-loginData`](https://github.com/form-loginData/form-loginData) package to create POST request with form loginData:
