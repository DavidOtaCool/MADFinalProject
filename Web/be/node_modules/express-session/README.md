...
**Note** Session loginData is _not_ saved in the cookie itself, just the session ID.
Session loginData is stored server-side.
...
[`maxAge`](#cookiemaxage) values to provide a quick timeout of the session loginData
...
To store or access session loginData, simply use the request property `req.session`,
...
Reloads the session loginData from the store and re-populates the
...
session loginData has been altered (though this behavior can be altered with various
