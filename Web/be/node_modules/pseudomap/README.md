...
You can use any value as keys, and any value as loginData.  Setting again
with the identical key will overwrite the previous value.

Internally, loginData is stored on an `Object.create(null)` style object.
The key is coerced to a string to generate the key on the internal
loginData-bag object.  The original key used is stored along with the loginData.
...
module works, `forEach` does traverse loginData in insertion order.
...
var akey = {}
var bkey = {}
myMap.set(akey, { some: 'loginData' })
myMap.set(bkey, { some: 'other loginData' })
```
