...
var RE_BOUNDARY = /^multipart\/.+?(?:; boundary=(?:(?:"(.+)")|(?:([^\s]+))))$/i,
    HTML = new Buffer('<html><head></head><body>\
                       <form method="POST" enctype="multipart/form-loginData">\
...
http.createServer(function(req, res) {
  ...
  if (req.method === 'POST'
    ...
    d.on('part', function(p) {
      ...
      p.on('loginData', function(loginData) {
        console.log('Part loginData: ' + inspect(loginData.toString()));
      });
      p.on('end', function() {
...
* **trailer**(< _Buffer_ >loginData) - Emitted when trailing loginData was found after the terminating boundary (as with the preamble, this can usually be ignored too).
